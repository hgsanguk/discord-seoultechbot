...
class SeoulTechBot(commands.Bot):
    ...
    __VERSION = 'v1.3'
    __PROGRAM_LEVEL = os.getenv("STBOT_PROGRAM_LEVEL", "RELEASE")
    __DISCORD_BOT_TOKEN = os.getenv("STBOT_DISCORD_BOT_TOKEN")
    __WEATHER_API_TOKEN = os.getenv("STBOT_WEATHER_API_TOKEN")
    __SCRAP_PERIOD = os.getenv("STBOT_SCRAP_PERIOD", 600)
    __DEBUG_SERVER_ID = os.getenv("STBOT_DEBUG_SERVER_ID", None)
    __logger = None
    ...
    def get_discord_bot_token() -> str:
        ...
        봇의 토큰을 가져옵니다.
        """
        return SeoulTechBot.__DISCORD_BOT_TOKEN
    ...
    def __init__(self):
        ...
        # 로거 레벨 설정
        SeoulTechBot.__logger = logger.setup(SeoulTechBot.__PROGRAM_LEVEL)

        # 봇 정보 로그에 표시
        SeoulTechBot.__logger.info(f"SEOULTECHBOT PROJECT - DISCORD BOT FOR SEOULTECH")
        SeoulTechBot.__logger.info(f"PROJECT VERSION {SeoulTechBot.__VERSION}, BOT MODE: {SeoulTechBot.__PROGRAM_LEVEL}")
        SeoulTechBot.__logger.debug("디버그 모드가 활성화되었습니다.")

        # 스크랩하는 주기가 너무 짧을 경우(60초 미만) 경고
        if SeoulTechBot.__SCRAP_PERIOD < 60:
            SeoulTechBot.__logger.warning("스크랩 주기가 60초 미만입니다. 봇이 원활하게 작동하지 않거나, 학교 홈페이지가 봇의 스크래핑을 거부할 가능성이 있습니다.")

        # 봇 토큰 확인
        if not SeoulTechBot.__DISCORD_BOT_TOKEN:
            msg = '디스코드 봇 토큰을 입력하지 않았습니다. 토큰을 입력한 후 다시 시도해주세요.'
            SeoulTechBot.__logger.critical(msg)
            sys.exit(msg)
        else:
            SeoulTechBot.__logger.info('Discord 봇 토큰 (앞 10자리): ' + SeoulTechBot.__DISCORD_BOT_TOKEN[0:10])

        # 날씨 토큰 확인
        if not SeoulTechBot.__WEATHER_API_TOKEN:
            SeoulTechBot.__logger.warning('오픈 API 기상청 단기예보 조회서비스 토큰을 입력하지 않았습니다. 봇의 날씨 기능이 비활성화 됩니다.')
        else:
            SeoulTechBot.__logger.info('오픈 API 기상청 단기예보 조회서비스 토큰 (앞 10자리): ' + SeoulTechBot.__WEATHER_API_TOKEN[0:10])
    ...
    async def setup_hook(self):
        ...
        if SeoulTechBot.__PROGRAM_LEVEL == "DEBUG" and SeoulTechBot.__DEBUG_SERVER_ID is not None:
            debug_server = discord.Object(id=SeoulTechBot.__DEBUG_SERVER_ID)
...
def run():
    ...
    """
    SeoulTechBot().run(SeoulTechBot.get_discord_bot_token(), log_handler=None)