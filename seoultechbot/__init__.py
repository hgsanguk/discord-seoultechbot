...
class SeoulTechBot(commands.Bot):
    ...
    async def __verify(self):
        ...
        """
        # 스크랩하는 주기가 너무 짧을 경우(120초 미만) 경고
        if self.__config.scrap_period < 120:
            self.__logger.error("스크랩 주기가 120초 미만입니다. 봇이 원활하게 작동하지 않거나, 학교 홈페이지가 봇의 스크래핑을 거부할 가능성이 있어 봇을 종료합니다.")
            sys.exit("스크랩 주기가 120초 미만입니다. 주기를 120초 이상으로 설정한 후 봇을 다시 실행해주세요.")
